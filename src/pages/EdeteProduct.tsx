...
import {  useUpdateProduct } from "@/features/AddProduct/hooks/useProductManagement";
import { useStore } from "@/features/store/hooks/UseStore";
import type { Offer, product } from "@/types";
import { handleImageUploadToImgbb } from "@/utils/uploadImage";
import {  Plus, X } from "lucide-react";
import { IoMdAdd } from "react-icons/io";import { useEffect, useState, type ChangeEvent, type KeyboardEvent } from "react";
import toast from "react-hot-toast";
import { useTranslation } from "react-i18next";
import { useNavigate, useParams } from "react-router-dom"
import OffersContainer from "@/features/AddProduct/components/OffersContainer";
import SaveModal from "@/components/ui/SaveModal";
import { useOneProducts } from "@/features/products/hooks/UseProducts";

const EdeteProduct = () => {
 const { id, product } = useParams();
  const router = useNavigate();
  const { t } = useTranslation("product");
const {data: theproduct, isLoading} = useOneProducts(product)
  // Fetch Store Data
  ...
  const { mutate: postproducts, isPending } = useUpdateProduct(id);
...
useEffect(()=>{
 setFormData({
    ...
    Oldprice: theproduct?.Oldprice || 0,
    store: id || "",
    type: theproduct?.type || "",
    show: theproduct?.show || true,
    subTitel: theproduct?.subTitel || "",
    tags: theproduct?.tags || [],
    LadingPages: theproduct?.LadingPages || [],
    images: theproduct?.images || [],
    Offers: theproduct?.Offers || [],
    colorOpions: theproduct?.colorOpions || [],
    sizeOpions: theproduct?.sizeOpions || [],
    subdomain: theproduct?.subdomain || store?.domain || ""
 })
 setOffers(theproduct?.Offers || [])
 setImages(theproduct?.images || [])
 setLandingPages( theproduct?.LadingPages || [])
 setTags(theproduct?.tags || [])
 setColorOpions( theproduct?.colorOpions || [])
 setSizeOpions(theproduct?.sizeOpions || [])
},[isLoading])

if(isLoading){
    return <div>dkjkajakj</div>
}
